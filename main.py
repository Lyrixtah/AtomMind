"""Main script to train and run SmallScientificLLM."""
...
STABLE_START = False    # Set True if model training is complete and starting fine-tuning
WARMUP_EPOCHS = 3
MAX_GRAD_NORM = 1.0
...
for epoch in range(EPOCHS):
    ...
    lr = 1e-5 if STABLE_START and epoch < WARMUP_EPOCHS else 1e-4
    ...
    if TRAIN_CHAT:
        try:
            chat_examples = executor.generate_candidates(
                prompt = (
                    "Generate casual daily conversation examples "
                    "between two people talking about hobbies, "
                    "greetings, or daily life."
                ),
                num_candidates=5,
            )
        except (RuntimeError, ValueError) as e:
    ...
    for i, task_prompt in enumerate(auto_tasks):
        ...
        if STABLE_START and epoch < WARMUP_EPOCHS:
            ...
            # get contradictions, hallucinations, reasoning_quality
            metrics = critic.evaluate_metrics(top_candidate)
